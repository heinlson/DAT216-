...
public class RecipeBackendController {
    ...
    //TODO fråga handledare om recipesearchcontroller och vilken man ska skicka till flowPane.
    private void updateRecipeList(){
        RecipeSearchController rsc = new RecipeSearchController();
        rsc.getFlowPane().getChildren().clear();
        List<Recipe> recipes = getRecipes();
        while(recipes.iterator().hasNext()){
            rsc.getFlowPane().getChildren().add(new RecipeListItem(recipes.iterator().next(), rsc));
        }
    }
